...
(define (schedule-matches? s timestamp)
  ...
  (match-define (date second minute hour day month _ week-day- _ _ _)
...
(define (schedule-next s [timestamp (current-seconds)])
  (with-handlers ([(lambda (e)
                     (and (exn:fail? e)
                          ...
                          (regexp-match? #rx"  nearest above: " (exn-message e))))
                   (lambda (e)
                     (match-define (regexp #rx"nearest above: ([^ ]+)" (list _ (app string->number ts)))
                       (exn-message e))
                     (schedule-next s ts))])
    (let/ec esc
      (match-define (schedule local? seconds minutes hours days months week-days) s)
      ...
      (unless (and (=  next-day day)
        ...
        (define-values (next-year next-month) ;; noqa
          (if (or (< next-day day)
                  (> next-day month-duration))
              (+month year month)
              (values year month)))
      ...
      (unless (= next-week-day week-day)
        (define-values (next-year next-month next-day) ;; noqa
